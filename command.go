...
func transform(f flags) func([][]byte) ([][]byte, error) {
	if bool(f.decodeEnabled) {
		return decode(f.ignoreGarbageEnabled)
	}
...
func encode(cols wrapColumn) func([][]byte) ([][]byte, error) {
	return func(lines [][]byte) ([][]byte, error) {
		...
		encoded := encodedText(base64.StdEncoding.EncodeToString(bytes.Join(lines, newline)))
		return wrap(encoded, cols), nil
	}
}

// decode returns an accumulator that concatenates all input lines and decodes
// the result. When isIgnoreGarbage is set, non-alphabet bytes are stripped first.
func decode(isIgnoreGarbage base64IgnoreGarbageFlag) func([][]byte) ([][]byte, error) {
	return func(lines [][]byte) ([][]byte, error) {
		if len(lines) == 0 {
			return [][]byte{}, nil
		}
		joined := bytes.Join(lines, nil)
		if bool(isIgnoreGarbage) {
...
// encodedText is a base64-encoded byte stream awaiting line wrapping.
...
func wrap(encoded encodedText, cols wrapColumn) [][]byte {
	if cols == 0 {
...
// inputByte is a single byte of decode input checked against the base64 alphabet.
type inputByte byte

// isBase64Byte reports whether b belongs to the standard base64 alphabet,
// including the '=' padding character.
func isBase64Byte(b inputByte) bool {
	switch {
	case byte(b) >= 'A' && byte(b) <= 'Z', byte(b) >= 'a' && byte(b) <= 'z', byte(b) >= '0' && byte(b) <= '9':
		return true
	default:
